...
| HTTP 客户端 | 任意方法/请求头发请求、**绕过 CORS**；params/headers/body(json·raw·form)/auth(Bearer·Basic·自定义头)，环境变量 `{{var}}`、历史、保存、curl 导入/导出、响应美化与高亮搜索、可跳过 TLS 校验 |
| WebSocket | 连接 ws/wss、自定义握手头、文本/二进制收发、消息时间线、自动重连、**心跳保活**(Ping 帧/自定义消息)、**连接事件**(连上按序自动发送初始化消息) |
| SSH 终端 | SSH/SFTP 一体：连接管理（分组·克隆·选择性导出·主密码加密存储），多标签交互式终端 + **本地终端**(PowerShell/cmd)，SFTP 浏览/上传/下载，trzsz，搜索·复制粘贴·主机指纹 TOFU·断线退避自动重连·可自定义快捷键 |
...
│  ├─ routes/+layout.svelte   全局样式 + 主题挂载（原生标题栏跟随夜间模式）+ 禁用原生右键菜单
│  ├─ routes/+page.svelte     应用外壳：侧栏 + 主面板（工具 keep-alive 保留内容）
│  ├─ app.css                 Tailwind 入口与基础样式
│  └─ lib/
│     ├─ ipc.ts               通用工具命令的强类型封装（invoke）
│     ├─ ssh.ts               SSH/SFTP/本地终端 命令与流式 Channel 封装
│     ├─ net.ts               HTTP / WebSocket 客户端命令封装
│     ├─ keys.svelte.ts       SSH 终端快捷键管理（长键/短键，可改可持久化）
│     ├─ resize.ts            拖拽改宽 action（侧栏/连接列表宽度）
│     ├─ persist.svelte.ts    工具输入跨重启持久化（localStorage）
│     ├─ state.svelte.ts      全局状态（当前工具 / 主题 / 固定 / 侧栏收起与宽度，均持久化）
│     ├─ tools.ts             工具注册表（新增工具只改这一处）
│     ├─ ui.ts                共享 Tailwind 类名片段
│     ├─ components/          Sidebar / ToolPanel / SegmentedControl / ResultRow / Stepper /
│     │  │                    CopyButton / Icon / ComboBox / TzSelect / JsonTree / RsaKeygen
│     │  └─ ssh/              ConnForm / TermView(xterm.js) / SftpPanel / ContextMenu / KeyBindings
│     └─ tools/               各工具视图组件（含 SshTerminal / HttpClient / WsClient）
├─ src-tauri/                 后端（Rust）
...
│  │                          zhconvert/crypto/vault/sshconfig/httpclient
│  ├─ src/lib.rs              #[tauri::command] 薄包装 + 应用装配（含 WebView2 省内存参数）
│  ├─ src/ssh/                SSH 有状态层：会话管理 + russh 客户端 + SFTP + 本地终端(ConPTY)
│  ├─ src/net/               HTTP 有状态层：reqwest 请求 + tokio-tungstenite WebSocket 会话
│  ├─ src/main.rs             入口
│  ├─ capabilities/           Tauri v2 权限（core + clipboard 读写 + dialog + window 主题）
│  └─ tauri.conf.json         窗口与打包配置
├─ docs/ssh-terminal/         SSH 工具的规格(spec) 与 实施计划(plan)
├─ docs/http-client/          HTTP / WebSocket 客户端的规格(spec) 与 实施计划(plan)
...
- **SSH 终端**（有状态工具）：后端 `russh` + `russh-sftp`（**ring 后端 + GNU 工具链，不依赖 MSVC/NASM**），
  会话经常驻会话表管理、退出即自清理；前端 `xterm.js`，I/O 走 Tauri Channel 流式推送。本地终端用 `portable-pty`（ConPTY）拉起 PowerShell/cmd，复用同一套会话读写/关闭。
  密码 / 私钥 / 口令用主密码经 **Argon2id 派生 + 字段级 AES-256-GCM** 加密落盘（仅密文这三类，明文连接信息照常可读）。详见 `docs/ssh-terminal/`。
- **HTTP / WebSocket 客户端**：后端 `reqwest` + `tokio-tungstenite`（均 **rustls**，契合 GNU 工具链），从而绕过 webview 的 CORS、可设任意方法与握手头、看原始响应；WS 为长连接会话，复用「会话表 + Channel 流式 + 断开自清理」模式。纯逻辑（变量替换/URL 拼接/curl 解析）在 `toolset-core::httpclient` 带单测。详见 `docs/http-client/`。
